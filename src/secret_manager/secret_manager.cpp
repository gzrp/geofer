...
SecretDetails get_openai_secret_details() {
    return {"openai", "geofer", "openai://", {"base_url", "api_key"}, {"api_key"}, {"api_key"}};
}

SecretDetails get_azure_secret_details() {
    return {"azure_llm", "geofer", "azure_llm://", {"api_key", "resource_name", "api_version"}, {"api_key"}, {"api_key", "resource_name", "api_version"}};
}

SecretDetails get_ollama_secret_details() {
    return {"ollama", "geofer", "ollama://", {"api_url"}, {"api_url"}, {"api_url"}};
}
...
duckdb::unique_ptr<duckdb::KeyValueSecret> SecretManager::ConstructBaseSecret(duckdb::vector<std::string>& prefix_paths_p, const SecretDetails& details,
...
void SecretManager::ValidateRequiredFields(const duckdb::CreateSecretInput& input,
    ...
    for (const auto& field: required_fields) {
        if (auto lookup = input.options.find(field); lookup == input.options.end() || lookup->second.ToString().empty()) {
