...
void Model::LoadModelDetails(const nlohmann::json& model_json) {
    ...
    model_details_.model = model_json.contains("model") ? model_json.at("model").get<std::string>() : std::get<0>(query_result);
    model_details_.provider_name = model_json.contains("provider") ? model_json.at("provider").get<std::string>() : std::get<1>(query_result);
    ...
    model_details_.context_window = model_json.contains("context_window") ? std::stoi(model_json.at("context_window").get<std::string>()) : std::get<2>(query_result);
    model_details_.max_output_tokens = model_json.contains("max_output_tokens") ? std::stoi(model_json.at("max_output_tokens").get<std::string>()) : std::get<3>(query_result);
...
std::tuple<std::string, std::string, int32_t, int32_t> Model::GetQueriedModel(const std::string& model_name) {
    const std::string query = duckdb_fmt::format(" SELECT model, provider_name, model_args "
                           " FROM geofer_storage.geofer_config.GEOFER_MODEL_USER_DEFINED_INTERNAL_TABLE "
                           " WHERE model_name = '{}' "
                           " UNION ALL "
...
nlohmann::json Model::CallEmbedding(const std::vector<std::string>& inputs) {
