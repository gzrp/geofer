...
void PromptParser::ParseCreatePrompt(Tokenizer& tokenizer, std::unique_ptr<QueryStatement>& statement) {
    ...
    std::string catalog;
    // CREATE GLOBAL|LOCAL PROMPT ( prompt_name, prompt_text);
...
void PromptParser::ParseUpdatePrompt(Tokenizer& tokenizer, std::unique_ptr<QueryStatement>& statement) {
    auto token = tokenizer.NextToken();
    auto value = duckdb::StringUtil::Upper(token.value);
    // UPDATE PROMPT prompt_name TO GLOBAL | LOCAL;
...
std::string PromptParser::ToSQL(const QueryStatement& statement) const {
    ...
    case StatementType::CREATE_PROMPT: {
        ...
        auto result = con.Query(duckdb_fmt::format(" SELECT prompt_name "
                                                   " FROM {}geofer_config.GEOFER_PROMPT_INTERNAL_TABLE"
                                                   " WHERE prompt_name = '{}';",
                                                   create_stmt.catalog.empty() ? "geofer_storage." : "",
                                                   create_stmt.prompt_name));
        if (result->RowCount() != 0) {
            throw std::runtime_error(duckdb_fmt::format("Prompt '{}' already exist.", create_stmt.prompt_name));
        }
        query = duckdb_fmt::format(" INSERT INTO {}geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
                                   " (prompt_name, prompt) "
                                   " VALUES ('{}', '{}'); ",
    ...
    case StatementType::DELETE_PROMPT: {
        ...
        auto con = Config::GetConnection();
        auto result = con.Query(duckdb_fmt::format(" DELETE FROM geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
                                                   " WHERE prompt_name = '{}'; ",
                                                   delete_stmt.prompt_name));

        query = duckdb_fmt::format(" DELETE FROM geofer_storage.geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
                                   " WHERE prompt_name = '{}'; ",
                                   delete_stmt.prompt_name);
        break;
    }
    case StatementType::UPDATE_PROMPT: {
        ...
        auto catalog = result->GetValue(1, 0).ToString() == "global" ? "geofer_storage." : "";
        query = duckdb_fmt::format(" INSERT INTO {}geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
    ...
    case StatementType::UPDATE_PROMPT_SCOPE: {
        ...
        con.Query(duckdb_fmt::format(" INSERT INTO {}geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
                                   " (prompt_name, prompt, updated_at, version) "
                                   " SELECT prompt_name, prompt, updated_at, version "
                                   " FROM {}geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
                                   " WHERE prompt_name = '{}';",
                                   update_stmt.catalog,
                                   update_stmt.catalog == "geofer_storage." ? "" : "geofer_storage.",
                                   update_stmt.prompt_name));

        query = duckdb_fmt::format(" DELETE FROM {}geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
                                 " WHERE prompt_name = '{}'; ",
                                 update_stmt.catalog == "geofer_storage." ? "" : "geofer_storage.",
                                 update_stmt.prompt_name);
        break;
    ...
    case StatementType::GET_ALL_PROMPT: {
        query = " SELECT 'global' as scope, t1.* "
                ...
                " FROM geofer_config.GEOFER_PROMPT_INTERNAL_TABLE AS t1 "
                " JOIN (SELECT prompt_name, MAX(version) AS max_version "
                " FROM geofer_config.GEOFER_PROMPT_INTERNAL_TABLE "
                " GROUP BY prompt_name) AS t2 "
                " ON t1.prompt_name = t2.prompt_name "
